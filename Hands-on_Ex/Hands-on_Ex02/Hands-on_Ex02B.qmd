...
No `glocal` anywhere in the list it prints, only `global`, defaulting to `FALSE`. To confirm this rather than just trust the printout, I ran the same envelope three ways on Choa Chu Kang with the same seed each time.
